...
html,body{margin:0;background:transparent;overflow:hidden;font-family:'Segoe UI','Segoe UI Emoji','Noto Color Emoji',sans-serif;color:white}
.m.cp{background:#2a1f42;border-left:3px solid #c9a6ff;padding:2px 6px;border-radius:4px}
.cpr{color:#c9a6ff;font-weight:800}
.m.cp::before{content:'CHANNEL POINTS ';color:#c9a6ff;font-weight:800;letter-spacing:1px;font-size:0.8em}
...
PopoutChat::PopoutChat(QWidget*p):QWidget(p){
    ...
    // Emote pictures arrive asynchronously; binding the document lets them be
    // added as resources the moment each download finishes.
    EmoteImageCache::instance().bind(chat_->document());
...
void PopoutChat::appendMessage(const ChatMessage&m){
    ...
    if(isChannelPointRedemption(m)){
        // Drawn as its own tinted, accented block. Chat lines here are
        // deliberately unfilled, so a filled row cannot be produced by typing.
        QTextBlockFormat redemptionFormat=cursor.blockFormat();
        redemptionFormat.setBackground(QColor(QString::fromLatin1(kRedemptionBackground)));
        redemptionFormat.setLeftMargin(6);
        cursor.setBlockFormat(redemptionFormat);
        const QString input=m.metadata.value(QStringLiteral("reward_input")).toString();
        cursor.insertHtml(QString("<span style='color:%1;font-weight:800;letter-spacing:1px'>CHANNEL POINTS</span> "
                                  "%2%3 <span style='color:%1;font-weight:700'>redeemed %4</span>%5")
            .arg(QString::fromLatin1(kRedemptionAccent),
                 platformIcon,chatNameHtml(m),redemptionRewardHtml(m),
                 input.isEmpty()?QString():QStringLiteral(" <span style='color:#ffffff;font-weight:600'>&#8212; ")+input.toHtmlEscaped()+QStringLiteral("</span>")));
    } else
    ...
    QTextCharFormat appearance;appearance.setFontFamilies({fontFamily_,QStringLiteral("Segoe UI Emoji"),QStringLiteral("Noto Color Emoji")});appearance.setFontPointSize(fontSizePoints_);
...
void PopoutChat::showTikTokActivity(const StreamEvent&e){
    // One line per activity kind. Anything unrecognised falls back to the
    // sentence TikTok itself showed, so a new kind is still readable.
    static const QHash<QString,QString> phrases{
        {QStringLiteral("tiktok_join"),QStringLiteral("joined the LIVE")},
        {QStringLiteral("tiktok_like"),QStringLiteral("liked the LIVE")},
        {QStringLiteral("tiktok_follow"),QStringLiteral("followed")},
        {QStringLiteral("tiktok_gift"),QStringLiteral("sent a gift")},
        {QStringLiteral("tiktok_share"),QStringLiteral("shared the LIVE")}};
    QString phrase=phrases.value(e.kind);
    if(phrase.isEmpty()){
        phrase=e.message.trimmed();
        if(phrase.startsWith(e.user))phrase=phrase.mid(e.user.size()).trimmed();
        if(phrase.isEmpty())phrase=QStringLiteral("is here");
    }
    QTextCursor cursor(chat_->document());cursor.movePosition(QTextCursor::End);
    if(!chat_->document()->isEmpty())cursor.insertBlock();
    const QString icon=showPlatformIcons_?QStringLiteral("<img src=':/brand/tiktok.png' width='18' height='18' style='vertical-align:middle;margin-right:5px'>"):QString();
    cursor.insertHtml(QStringLiteral("%1<span style='color:#7f8ba5;font-style:italic'><b style='color:#18e0d5'>%2</b> %3</span>")
                          .arg(icon,e.user.toHtmlEscaped(),phrase.toHtmlEscaped()));
    chat_->moveCursor(QTextCursor::End);chat_->ensureCursorVisible();trimChatBlocks(chat_->document());
}
...
void PopoutChat::applyTextOutline(){
    if(!chat_)return;
    // setFamilies (not setFamily) gives per-glyph fallback, so an emoji inside
    // a TikTok display name renders from the emoji font instead of as a box.
    QFont font(fontFamily_);font.setPointSize(fontSizePoints_);
    font.setFamilies({fontFamily_,QStringLiteral("Segoe UI Emoji"),QStringLiteral("Noto Color Emoji")});
    chat_->setFont(font);chat_->document()->setDefaultFont(font);
    QTextCursor cursor(chat_->document());cursor.select(QTextCursor::Document);
    QTextCharFormat format;
    format.setFontFamilies({fontFamily_,QStringLiteral("Segoe UI Emoji"),QStringLiteral("Noto Color Emoji")});format.setFontPointSize(fontSizePoints_);
