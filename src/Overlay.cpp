...
void PopoutChat::appendMessage(const ChatMessage&m){
    ...
    cursor.insertHtml(QString("<span style='background-color:transparent;color:#ffffff'>%1%2<b style='color:%3'>%4</b> <span style='color:#ffffff;font-weight:600'>%5</span></span>")
        .arg(platformIcon,badges.isEmpty()?QString():badges+QStringLiteral(" "),m.color.name(),m.user.toHtmlEscaped(),m.text.toHtmlEscaped()));
